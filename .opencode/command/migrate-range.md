...
fresh reviewer, fix required findings in a separate fresh context, and independently re-review. Treat changes created after a worker invocation as task-attributable unless there is positive evidence of an external writer; staged versus
...
determinable. Stop on genuine blockers, failed prerequisites, exhausted fix cycles, or task-to-task diff boundaries that remain ambiguous after this recovery. Stop after `$2` and do not begin tasks outside this range.
