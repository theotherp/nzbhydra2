...
| # | Item | Where | Status |
|---|------|-------|--------|
| 21 | Collapse the three history pages' ~140 duplicated lines each into a `useHistoryPage` hook plus a frame component. | `ui-react/src/features/stats/history/{Search,Download,Notification}HistoryPage.tsx` | done (d29d4922e) |
| 22 | One `useListEditorTransaction` hook for the five config list editors; `CustomMappingsSection` has no token guard and `NotificationEntriesSection` only `write`. | `ui-react/src/features/config/{auth/AuthUsersSection,categories/CategoriesTable,downloading/DownloadersSection,external-tools/ExternalToolsSection,indexers/IndexersConfigTab,searching/CustomMappingsSection,notifications/NotificationEntriesSection}.tsx` | done (295b27ab8) |
| 23 | Split `SearchResults.tsx` (2783 lines) at verified seams: `ResultsAlerts` (1243-1319), `ResultsToolbar` (1320-1702), `ResultsTable` (1745-2588), `ResultsPagingFooter` (2601-2632), plus `useResultDisplayChoices` and `useResultSelection` hooks. | `ui-react/src/features/search/results/SearchResults.tsx` | done (f9988b3c5) |
| 24 | Downloader scaffolding: `addBySearchResultIds` is a 115-line method with a dead null check, wrong error iteration and overwritten messages; error throttle and offline fallback copy-pasted between Sabnzbd and NzbGet; null Torbox torrent id NPEs inside a swallowed catch. | `downloading/downloaders/Downloader.java:101-217, 326-352`, `sabnzbd/Sabnzbd.java:194-198, 245-253`, `nzbget/NzbGet.java:181-185, 234-246`, `torbox/Torbox.java:161-174` | done (25fcfbc37) |
| 25 | `checkIfHitLimitIsExceeded`: `oldestAccessFromApi` is always true so the else branch is dead; split into two functions with a result type. Also the swapped format arguments at line 192 (and `SearchResultAcceptor.java:388`). | `searching/IndexerForSearchSelector.java:192, 336-438` | done (c2a401db9) |
...
- Resolved 2026-09-09 (0e3190d45): the two Playwright specs reported as flaky (`news.spec.ts:25`, `focus-indication.spec.ts:1362`) were failing on the browser-diagnostics fixture because `GET /internalapi/updates/infos` answered 500 when the blocked-versions list on GitHub was unreachable; the fetch is now non-fatal and the rig points it at the mockserver. The reported runner exit-code gap did not reproduce: probed with a plain assertion failure and a fixture-teardown failure, `misc/run_gui_systemtest.py` exits 1 for both.
- `Downloader.addBySearchResultIds` (item 24 review): any `DownloaderException` from a single add, including Torbox answering a usenet add without a download id, aborts the whole batch and marks the remaining entries failed. Consistent with the pre-existing handling of transport errors, but a malformed per-item response arguably wants an item-level failure. Also `successful = !addedNzbs.isEmpty()` reports false when every id was invalid and the downloader was never contacted, contradicting `AddNzbsResponse`'s javadoc.
