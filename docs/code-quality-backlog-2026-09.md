...
- `misc/run_gui_systemtest.py` (observed 2026-09-09 during the external-API runs): the runner exits 0 when the Playwright phase reports a failure (`1 failed / 233 passed` twice, exit code 0 both times), so a green exit is not evidence of a green Playwright phase. Also two specs flaked once each across the two runs: `news.spec.ts:25` (news overflow) and `focus-indication.spec.ts:1362` (MUI Link / error Button focus ring). Propagate the Playwright result into the exit code and look at the two specs.
