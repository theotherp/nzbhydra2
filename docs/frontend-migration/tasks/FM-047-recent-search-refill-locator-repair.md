...
- **The role is the only thing that was wrong.** Confirm this rather than assume it: Playwright 1.62.1's `name` option matches the accessible name as a **case-insensitive substring** by default — `playwright-core/types/types.d.ts`,
  the `name?: string|RegExp` option on `Page.getByRole`, documented as *"Option to match the accessible name. By default, matching is case-insensitive and searches for a substring, use `exact` to control this behavior."* Cited by symbol
  and text rather than by line number: that block appears eight times across the `Page`, `Frame`, `Locator`, and `FrameLocator` variants and its coordinates move between releases, which is how this packet's original `:3906`/`:8115`
  citation came to point at `hasNotText` instead. So the existing `{name: "Refill"}` already matched; and `queryRole` in `playwright-core/lib/coreBundle.js` matches `getAriaRole(element)` per element with **no accessibility-tree pruning and no children-presentational
