...
### 2026-09-07 — Widen the phone results toolbar and row-action gaps for touch

- **Why not a packet:** styling inside existing features — two `sx` gap values, no behavior, contract, or `data-testid` change. Rendering change referenced by `F-SEARCH-RESULTS/phone-toolbar-gap-after.png`, `phone-row-actions-gap-after.png`, and `phone-results-gap-after-full.png` (git-ignored strip, 390x844, grey theme).
- **Paths:** `core/ui-react/src/features/search/results/SearchResults.tsx`
- **Gates:** `core/ui-react` `typecheck`, `lint` (0 errors, 16 pre-existing warnings), `format:check`, `test -- --run` (139 files / 1983 tests), `build`, `validate:migration` green; `git diff --check` clean. `npm install` skipped (lockfile unchanged). No system spec asserts these gaps, so none was rerun; captures were taken from the running vite dev server proxied to the `--keep-services` systemtest Hydra left by FM-188.
- **Commit:** `617c2d5a8`
- **Note:** owner report after FM-188. The toolbar's Display / Filters / Save search cluster (`ml: "auto"` wrapper, compact branch) had no gap; the `<768px` card-layout block now sets `gap: 1` on the Actions cell's stacks (8px, up from 4px, and from the compact-rows branch's 2px) using the same `tbody` selector as that branch so it wins at equal specificity. Desktop values unchanged. The owner's aside — why the Filters button is wrapped in a `span` while its neighbours are bare buttons — is MUI `Badge`'s root element carrying the active-filter count, not a different control.

### 2026-09-08 — Survive a slow first live-transport connect

- **Why not a packet:** single-module bugfix with regression tests that are red before the fix (`transport.test.ts`, `SearchPage.test.tsx`); no contract, `data-testid`, or persisted-data change. Implemented by a subagent and independently reviewed twice (first review found the search page never withdrew its warning after a late connect; fixed and re-reviewed).
- **Paths:** `core/ui-react/src/api/live/transport.ts`, `core/ui-react/src/api/live/transport.test.ts`, `core/ui-react/src/features/search/SearchPage.tsx`, `core/ui-react/src/features/search/SearchPage.test.tsx`
...
### 2026-09-08 — Thread React Query's abort signal through history, system and stats queries

- **Why not a packet:** mechanical plumbing inside existing features; every API function gained a trailing optional `AbortSignal` and fifteen `queryFn`s pass it. No request shape, URL, contract, `data-testid` or rendering change. Implemented by a subagent, reviewed by a fresh subagent (found one remaining caller of `getUpdateInfos` in the footer banners; converted).
- **Paths:** `core/ui-react/src/api/history/{request,downloads,notifications}.ts`, `src/api/{searchHistory,savedSearches,news}.ts`, `src/api/stats/indexerStatuses.ts`, `src/api/system/{logs,tasks,backup,updates}.ts`, the calling pages under `src/features/{stats,system,news}` and `src/app/status/UpdateFooterBanners.tsx`, plus five new tests.
...
### 2026-09-13 — Only advertise book search when the API can actually serve it

- **Why not a packet:** the React side is a single help string in the settings index; the actual fix is backend, in `CapsGenerator`. No component, contract, `data-testid` or rendering change. Issue #1083: book searches from Readarr returned nothing with the default `searching.generateQueries = INTERNAL`, because no usenet indexer supports book searches natively, yet the caps unconditionally advertised `book-search available="yes"`.
- **Paths:** `core/src/main/java/org/nzbhydra/api/CapsGenerator.java`, `core/src/test/java/org/nzbhydra/api/CapsGeneratorTest.java`, `core/src/main/resources/changelog.yaml`, `core/ui-react/src/features/config/settingsSearch/settingsIndex.ts`
- **Gates:** `mvn -pl core -DskipTests=false -Dtest=CapsGeneratorTest,IndexerForSearchSelectorTest test` (38 tests, 0 failures); `core/ui-react` `typecheck`, `lint` (0 errors, 16 pre-existing warnings), `format:check`, vitest for `features/config/settingsSearch` and `features/config/searching` (3 files / 111 tests) green; `git diff --check` clean.
- **Commit:** `46a8aa482`
- **Note:** the caps now report `book-search available="no"` with empty supported params unless query generation applies to API searches (`API`, `BOTH` or `ALL_BUT_RSS`; `ONLY_RSS` and `INTERNAL` count as no, mirroring `SearchRequest.meets` for an API book search that carries a query or title) or a usable indexer (state `ENABLED` or `DISABLED_SYSTEM_TEMPORARY`, enabled for API searches) lists `BOOK` in its supported search types. The JSON caps derive from the same XML object, so both paths change together. The default value of `searching.generateQueries` was deliberately left at `INTERNAL`. Implemented by a subagent, reviewed by a fresh subagent (no required findings; its minor findings, missing `ALL_BUT_RSS`/`ONLY_RSS` tests and the indexer branch ignoring the indexer's own search-source restriction, were fixed). Recorded, not fixed: a torznab indexer listing `BOOK` makes the newznab caps say yes although the selector rejects torznab indexers for NZB API searches; no real indexer does this.

### 2026-09-13 — Category mapping: per-indexer result cache removed, language-variant ids mapped

- **Why not a packet:** backend only, no React change. Follow-up of an analysis of how newznab categories are converted between API searches, the global categories config and the per-indexer `IndexerCategoryConfig`; conclusion was to keep the design (global standard-id table + five per-indexer subtype ids + numeric fallback) and fix what was actually broken.
- **Paths:** `core/src/main/java/org/nzbhydra/indexers/NewznabCategoryComputer.java`, `core/src/main/java/org/nzbhydra/searching/CategoryProvider.java`, their tests, `core/src/main/resources/changelog.yaml`
- **Gates:** `mvn -pl core -DskipTests=false -Dtest=NewznabCategoryComputerTest,CategoryProviderTest,NewznabTest,ExternalApiTest test` (2 + 8 + 43 + 6 tests, 0 failures); `git diff --check` clean.
- **Commit:** `e701f9fa3` (cache removal), `24c0fa0a2` (hundreds-digit variant mapping), `14be683de` (eager fallback)
- **Note:** the result category cache was a singleton keyed by newznab id although the closure used the indexer's own subtype ids and Category instances are replaced on config save; removed rather than scoped. Ids 1000-9999 with a hundreds digit (treasure-maps 2140) are now tried as their standard number (2040) before the main-category fallback, on both the result and the incoming-API path; 5170 "Anime DE" therefore lands in Anime instead of TV, accepted. Searching those variants still needs the ids in the categories config; sending ids to indexers is not filtered against their caps (unverified whether unknown ids are rejected). Analysis reviewed by an opus subagent (corrected fixture details and the placement of the variant check), diffs reviewed by a sonnet subagent (no required findings). Recorded, not fixed: MAGAZINE subtype is detected at caps time but unreachable from the UI; default Ebook category includes 8010.

### 2026-09-13 — Backup retention never deleted anything without a filesystem birth time (#1070)

- **Why not a packet:** backend only. `BackupAndRestore.getExistingBackups` dated backups by the filesystem creation time, which is epoch 0 on Btrfs/NFS/SMB mounts, so the "no successful newer backup" guard in `deleteOldBackupFiles` tripped for every file and retention never ran.
- **Paths:** `core/src/main/java/org/nzbhydra/backup/BackupAndRestore.java`, `core/src/test/java/org/nzbhydra/backup/BackupAndRestoreTest.java` (new), `core/src/main/resources/changelog.yaml`
- **Gates:** `mvn -pl core -DskipTests=false -Dtest=BackupAndRestoreTest,BackupTaskTest test` (7 + 6 tests, 0 failures); `git -c core.whitespace=cr-at-eol diff --check` clean.
- **Commit:** this commit
- **Note:** timestamp precedence is now filename (`nzbhydra-yyyy-MM-dd HH-mm-ss.zip`, the pattern Hydra writes), then creation time if not epoch, then last modified; the same helper feeds the backup list API and the retention run. Implemented by a sonnet subagent in an isolated worktree (based on an old commit, cherry-picked; its `mvn install` of `shared/` from that base overwrote `~/.m2` and had to be redone from the main tree), reviewed by a fresh sonnet subagent: no required findings; its minor `@Reflective` finding was fixed, the test's hand-rolled reflection field setter instead of `@InjectMocks` was left as is.

### 2026-09-13 — Downloader status footer shows free disk space and a "queue does not fit" warning (#1060)

- **Why not a packet:** owner asked for a direct implementation of issue #1060; an additive strip on the existing `C-DOWNLOADER-STATUS` footer whose data comes from new optional fields on the backend's `DownloaderStatus`, tolerated as absent by the zod schema like every other field on that message. Two new `data-testid`s (`downloader-status-free-space`, `downloader-status-space-warning`), none changed.
- **Paths:** `shared/mapping/.../downloaders/{DownloaderStatus,Converters}.java`, `.../sabnzbd/mapping/Queue.java`, `core/.../downloaders/{sabnzbd/Sabnzbd,nzbget/NzbGet}.java` and their tests, `core/openapi.json`, `core/src/main/resources/changelog.yaml`, `core/ui-react/src/api/live/downloaderStatus.ts`, `core/ui-react/src/api/generated/openapi.ts`, `core/ui-react/src/app/status/{DownloaderStatusFooter.tsx,downloaderFooter.ts}` and their tests
...
### 2026-09-13 — Even out the vertical rhythm of config rows

- **Why not a packet:** styling inside `C-CONFIG-FIELDS`' shared row; no contract or behaviour change. Owner report with screenshots: the gaps between fields read as cramped, and raising the row margin alone (tried in dev tools) made switch rows drift further apart than field rows.
- **Paths:** `core/ui-react/src/features/config/components/{SettingRow,SwitchSetting}.tsx`, `core/ui-react/src/features/config/indexers/IndexersConfigTab.test.tsx` (the pinned margin)
- **Gates:** `core/ui-react` `typecheck`, `eslint` on the components dir (0 errors), `prettier --check`, `validate:focus-affordances`, vitest `features/config` (30 files / 711 tests) green; checked on the running instance at 1280px (`Replace umlauts` switch row now sits the same distance from its neighbours as the fields around it).
- **Commit:** this commit
...
### 2026-09-17 — Heap dump works on native builds: own endpoint instead of `actuator/heapdump`

- **Why not a packet:** owner-directed single fix of a button that could never work on the released binaries. It does move an API-SYSTEM-* address (`/actuator/heapdump` → `/internalapi/debuginfos/heapdump`), which the gate criteria put in packet territory; recorded here as a deviation on the owner's instruction. The link is built and followed by the browser only, so no component consumes a response shape, and `APIS.yaml` is updated with the new path, contract state and evidence.
- **Paths:** `core/src/main/java/org/nzbhydra/debuginfos/{DebugInfosProvider,DebugInfosWeb}.java`, `core/pom.xml`, `core/openapi.json`, `core/src/main/resources/changelog.yaml`, `core/ui-react/src/api/system/debug.ts` and its test, `core/ui-react/src/api/generated/openapi.ts`, `core/ui-react/src/features/system/bugreport/SystemBugreportTab.test.tsx`, `tests/system/src/main/java/org/nzbhydra/HydraClient.java`, `tests/system/src/test/java/org/nzbhydra/DebugInfosTest.java`, `tests/system/tests/system.spec.ts`, `docs/frontend-migration/APIS.yaml`
- **Gates:** `mvn -o -pl core package -DskipTests`; `mvn -o -pl org.nzbhydra.tests:system -DskipTests=false -Dtest=DebugInfosTest -Dnzbhydra.port=5077 test` (5 tests, 1 the pre-existing `@Disabled`, against a locally started instance) — this includes the new `shouldCreateHeapDump`; `core/ui-react` `typecheck`, `eslint` (0 errors), `prettier --check`, vitest `api/system/debug.test.ts` + `features/system/bugreport` (36 tests); `npm run generate:api` re-run after `core/openapi.json`; manual `curl` against a running JVM instance (200, `nzbhydra-heapdump-….hprof`, `JAVA PROFILE 1.0.2`, temp file gone afterwards); a GraalVM 25.2.4 native-image probe carrying both code paths, built and run.
- **Commit:** this commit
- **Note:** Spring Boot's `actuator/heapdump` only knows `HotSpotDiagnosticMXBean` and answers 503 in a native image, which is what nearly every installation runs. `DebugInfosProvider.createHeapDump` picks `VMRuntime.dumpHeap(path, true)` there and the MXBean on a JVM, deciding on the `org.graalvm.nativeimage.imagecode` property rather than `NzbHydra.isNativeBuild()` — the latter reports what the environment claims, and only the actual runtime matters. `org.graalvm.sdk:nativeimage` is a `provided` dependency (class file v55, so the JDK 17 builds compile), and `--enable-monitoring=heapdump` was added to the native build args; measured on the probe, the flag costs nothing (identical binary size with and without it). The endpoint streams the dump and deletes the temp file even when the client aborts, which is what the new system test does: `HydraClient.getFirstBytes` reads 18 bytes and drops the connection instead of transferring the ~140 MB body. CI's Linux system-test job runs the Java suite against the native core in Docker, so the case that was broken is the case under test. Recorded, not fixed: `tests/system/tests/system.spec.ts` fails `prettier --check` on an import unrelated to these lines (pre-existing, from commit e29ae9ad7).

### 2026-09-18 — Title grouping in the search results became the "Group same titles" display option

- **Why not a packet:** it is a new user-observable capability, which the gate puts in packet territory; recorded here as a deviation on the owner's explicit routing after both routes were priced. The change is one module's grouping key
  (`resultTable.ts:groupingKey`) plus the option's wiring through the existing display-options path, with regression tests at both the unit and the browser level.
- **Paths:** `core/ui-react/src/features/search/results/{resultTable.ts,resultTable.test.ts,storedChoices.ts,useResultDisplayChoices.ts,ResultsPopovers.tsx,ResultsToolbar.tsx,SearchResults.tsx,SearchResults.test.tsx}`,
  `docs/frontend-migration/FEATURES.yaml` (the persisted-keys comment on `F-SEARCH-RESULTS`), `tests/system/tests/results.spec.ts`
- **Gates:** `core/ui-react` `typecheck`, `lint` (0 errors, 16 pre-existing warnings), `prettier --check` on the touched files, `test -- --run` (145 files, 2100 tests), `build`, `check:api`, `validate:migration`; `tests/system`
  `npx tsc --noEmit` and the whole `results.spec.ts` (37 passed) against a locally started instance (`misc/run_gui_systemtest.py --runtime local`); `git diff --check`. Pre-existing, not touched: `prettier --check` fails on
  `src/features/search/SearchPage.test.tsx` and `src/features/system/bugreport/` (proved against a stashed tree).
- **Visual gate:** `tests/system/visual-evidence/F-SEARCH-RESULTS/display-options-desktop.png` (regenerated by the run above), showing the new entry directly below "Group TV episodes".
- **Commit:** 4bf635a961002b887b5c648100f99584644a078f
...
- **Commit:** b3978ac51878fe7c556979ebb71d61b4c20ad963
...
### 2026-09-18 — "Expand groups by default" migrated from the old display menu

- **Why not a packet:** owner-routed as a quickfix after the previous two, same as those: it restores one never-migrated display option (`search-results-controller.js:199,289`) through the existing display-options path, in one module,
  with tests at both levels. Recorded here as a deviation, since a display option users can see is a capability by the gate's wording.
- **Paths:** `core/ui-react/src/features/search/results/{SearchResults.tsx,SearchResults.test.tsx,ResultsPopovers.tsx,ResultsToolbar.tsx,storedChoices.ts,useResultDisplayChoices.ts}`,
  `docs/frontend-migration/FEATURES.yaml` (the persisted-keys comment on `F-SEARCH-RESULTS`), `tests/system/tests/results.spec.ts`
- **Gates:** `core/ui-react` `typecheck`, `lint` (0 errors, 16 pre-existing warnings), `prettier --check`, `test -- --run` (145 files, 2104 tests), `build`, `check:api`, `validate:migration`; `tests/system` `npx tsc --noEmit` and the
  whole `results.spec.ts` (37 passed) against a locally started instance (`misc/run_gui_systemtest.py --runtime local`); `git diff --check`.
- **Visual gate:** `tests/system/visual-evidence/F-SEARCH-RESULTS/display-options-desktop.png` (regenerated by that run), showing the entry below "Group same titles".
- **Commit:** f5ea92f6c (`Expand groups by default`)
- **Note:** the expansion state inverted from "which groups are expanded" to "which groups differ from the option" (`titleExpansionOverrides`), so a group arriving with a later page follows the option without re-seeding; flipping the
  option clears the overrides, as legacy did by pushing the new value onto every row. Duplicate groups keep their own control and still start collapsed (legacy hardcoded `duplicatesExpanded = false` beside the line this option seeded).
  Owner request: the entry is hidden while neither grouping kind is on, where it could only be a no-op; it keeps its stored value while hidden.
- **Open candidate:** the one-shot "expand/collapse all visible groups" command discussed alongside this is *not* built. Placement agreed if it is wanted later: a toolbar icon button beside Display on desktop, and a command row at the
  top of the same popover on mobile (the phone chrome is one sticky row with no spare target, per FM-181).

### 2026-09-18 — "Group torrent and Usenet results" hidden while no grouping can form

- **Why not a packet:** owner's observation on the entry above, same mechanism and the same one-line condition: the option only decides whether a group may hold both kinds of result, so with both grouping options off it cannot change
  anything. Menu visibility only; the stored value and the grouping key are untouched.
- **Paths:** `core/ui-react/src/features/search/results/{ResultsPopovers.tsx,SearchResults.test.tsx}`
- **Gates:** `core/ui-react` `typecheck`, `lint` (0 errors, 16 pre-existing warnings), `prettier --check`, `test -- --run` (145 files, 2104 tests), `build`, `check:api`, `validate:migration`; `tests/system` `results.spec.ts`
  (37 passed) against a locally started instance; `git diff --check`.
- **Commit:** 970c61056
- **Note:** both entries now share one `groupingPossible` condition in `DisplayOptionsMenu`. Recorded, not fixed: `DuplicateDetector.testForSameness` excludes TORRENT but not TORBOX, so an NZB and a TORBOX result could in principle share
  a duplicate group that the download-type qualifier would then split — a backend question if it ever appears in practice, not a menu one.

### 2026-09-18 — Display options reordered so a toggle never moves itself

- **Why not a packet:** owner-reported UX defect in one array literal, with a regression test. No contract, selector or persisted value changes — only the order the entries are rendered in.
- **Paths:** `core/ui-react/src/features/search/results/{ResultsPopovers.tsx,SearchResults.test.tsx}`, `tests/system/tests/results.spec.ts`
- **Gates:** `core/ui-react` `typecheck`, `lint` (0 errors, 16 pre-existing warnings), `prettier --check`, `test -- --run` (145 files, 2105 tests), `build`, `check:api`, `validate:migration`; `tests/system` `npx tsc --noEmit` and the
  whole `results.spec.ts` (37 passed) against a locally started instance; `git diff --check`.
- **Visual gate:** `tests/system/visual-evidence/F-SEARCH-RESULTS/display-options-desktop.png` (regenerated by that run), showing the new order.
- **Commit:** 3b4b45f2a
