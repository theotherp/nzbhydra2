...
test.describe("External Tools Configuration", () => {
    test.beforeEach(async ({page, hydra}) => {
        ...
        await expect(
            page.getByRole("button", {name: /Add external tool/}),
        ).toBeVisible();
    });
    ...
    test("should display External Tools tab in configuration", async ({
        ...
        await expect(
            page.locator(".nav-tabs a").filter({hasText: "External Tools"}),
        ).toBeVisible();
    });

    test("should navigate to External Tools configuration", async ({page}) => {
        await expect(
            configurationSwitchGroup(page, "Sync on config change"),
        ).toBeVisible();
        await expect(
            page.getByRole("button", {name: /Add external tool/}),
        ).toBeVisible();
    ...
    test("should open external tool configuration modal with presets", async ({
        ...
        for (const preset of [
            ...
            await expect(
                page.locator(".dropdown-menu li").filter({hasText: preset}),
            ).toBeVisible();
        }
    });
    ...
    test("should open Sonarr preset configuration modal", async ({page}) => {
        ...
        await expect(
            page.getByRole("heading", {name: "External Tool Configuration"}),
        ).toBeVisible();
        await expect(modalField(page, "Name")).toHaveValue("Sonarr");
        await expect(modalField(page, "Host URL")).toHaveValue(
            "http://localhost:8989",
        );
        ...
        await expect(
            modalSwitch(page, "Enable automatic search"),
        ).toBeChecked();
        await expect(
            modalSwitch(page, "Enable interactive search"),
        ).toBeChecked();
    ...
    test("should save external tool configuration", async ({hydra, page}) => {
        ...
        await expect(
            page.getByRole("button", {
                name: "UI System Test Radarr Save",
                exact: true,
            }),
        ).toBeVisible();
        await expect(
            page.getByText(testEnvironment.radarrInternalUrl, {exact: true}),
        ).toBeVisible();
        await expect(
            page.locator(".btn-danger .glyphicon-remove"),
        ).toBeVisible();
    });
    ...
    test("should trigger manual sync all", async ({hydra, page}) => {
        await configurationSwitch(page, "Sync on config change")
            .locator(
                "xpath=ancestor::*[contains(@class, 'bootstrap-switch')][1]",
            )
            .click();
        await expect(
            configurationSwitch(page, "Sync on config change"),
        ).toBeChecked();
        ...
        await expect(
            page
                .locator(".growl-message")
                .filter({hasText: "Successfully synced to"}),
        ).toBeVisible();
    ...
    test("should edit existing external tool", async ({hydra, page}) => {
        ...
        await page
            .getByRole("button", {
                name: "UI System Test Radarr Edit",
                exact: true,
            })
            .click();
        await expect(
        ...
        await expect(
            page.getByRole("button", {
                name: "UI System Test Radarr Edited",
                exact: true,
            }),
        ).toBeVisible();
    });
    ...
    test("should delete external tool", async ({hydra, page}) => {
        ...
        await expect(
            page.getByRole("button", {
                name: "UI System Test Radarr Delete",
                exact: true,
            }),
        ).toBeHidden();
        await expect(
...
async function addRadarr(page: Page, name: string): Promise<void> {
    ...
    await modalField(page, "NZBHydra Host").fill(
...
function configurationSwitch(page: Page, label: string): Locator {
    return configurationSwitchGroup(page, label)
