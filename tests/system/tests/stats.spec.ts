...
test("should render React indexer statuses and canonical history tabs responsively", async ({page}, testInfo) => {
    ...
    await page.route("**/internalapi/indexerstatuses", async (route) => {
        await route.fulfill({
            ...
            body: JSON.stringify([
                {indexer: "Zulu", state: "DISABLED_SYSTEM_TEMPORARY", disabledUntil: "2030-01-02T00:00:00Z", lastError: "Quota reached", apiHits: 4, apiHitLimit: 5, downloadHits: 1, downloadHitLimit: 2, apiResetTime: "2030-01-03T00:00:00Z", vipExpirationDate: "2020-01-01"},
    ...
    await expect(page.getByRole("tab", {name: "Indexer statuses"})).toBeVisible();
    ...
    await expect(page.getByRole("table", {name: "Indexer statuses"})).toContainText("Quota reached");
