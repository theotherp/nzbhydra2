...
describe("IndexerStatusesPage", () => {
    ...
    // on a body cell has to name the header at the same column index, or a
    // later edit that adds/reorders a column silently mislabels the cards
    // below 768px. jsdom does not evaluate the media query the cards live
    // behind, so this checks the labelling contract only, not that the
    // layout actually stacks.
    it("should label every body cell with its own column's header text", async () => {
        renderPage(async () => ({
            malformedCount: 0,
            statuses: [{indexer: "Alpha", state: "ENABLED", apiHits: 3}],
        }));
        const table = await screen.findByRole("table", {
            name: "Indexer statuses",
        });
        assertCellLabelsMatchHeaders(table);
    });
});
