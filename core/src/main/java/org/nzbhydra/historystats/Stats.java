...
public class Stats {
    ...
    List<IndexerDownloadShare> indexerDownloadShares(final StatsRequest statsRequest) {
        ...
        Set<String> indexerNamesToInclude = searchModuleProvider.getIndexers().stream().filter(x -> x.getIndexerEntity().getState() == IndexerState.ENABLED || statsRequest.isIncludeDisabled()).map(Indexer::getName).collect(Collectors.toSet());
    ...
    List<IndexerApiAccessStatsEntry> indexerApiAccesses(final StatsRequest statsRequest) {
        ...
        Set<Integer> indexerIdsToInclude = searchModuleProvider.getIndexers().stream().filter(x -> x.getIndexerEntity().getState() == IndexerState.ENABLED || statsRequest.isIncludeDisabled()).map(x -> x.getIndexerEntity().getId()).filter(id -> indexerRepository.findById(id) != null).collect(Collectors.toSet());
