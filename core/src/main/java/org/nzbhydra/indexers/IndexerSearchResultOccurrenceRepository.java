...
public interface IndexerSearchResultOccurrenceRepository extends JpaRepository<IndexerSearchResultOccurrenceEntity, Integer> {
    ...
    @Modifying
    @Query(value = "MERGE INTO INDEXERSEARCHRESULTOCCURRENCE (ID, INDEXER_SEARCH_ID, SEARCH_RESULT_ID) " +
                   "KEY (INDEXER_SEARCH_ID, SEARCH_RESULT_ID) VALUES (NEXT VALUE FOR INDEXERSEARCHRESULTOCCURRENCE_SEQ, :indexerSearchId, :searchResultId)", nativeQuery = true)
